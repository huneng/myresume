...
import com.huneng.paint.AxisDisplay;
import com.huneng.paint.Histogram;
import com.huneng.paint.HistogramAxis;
import com.huneng.paint.Line;
import com.huneng.paint.LineAxis;
...
public class SimpleResume2 extends View {
	...
	public SimpleResume2(Context context) {
		super(context);

	}

	public SimpleResume2(Context context, int w, int h, MyJson data) {
		super(context);
		...
		axis = new AxisDisplay(0, level[0], width, level[1], w / xCount, start);
		rectAx = new HistogramAxis(0, level[0] - 200, width, level[0], xCount,
				10, start, 0);
		lineAx = new LineAxis(0, level[1], width, level[2] , xCount, 10,
				start, 0);
		t = level[2];
		if(h-level[2]>200)
			t = h-200;
        textsRec = new Rect(0, t, width, h);
		colorIndex = 0;
		mPaint = new Paint();
